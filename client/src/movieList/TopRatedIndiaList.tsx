...
import { TopRatedIndiaComponent } from '../movie-card/TopRatedIndiaComponent';
...
export const TopRatedIndiaList = () => {
  ...
  const fetchData = async(searchQuery:string)=>{
    try{
      const response = await axios.get(`http://localhost:3002/top-rated-india?title_like=${searchQuery}`)
  ...
  return (
      <Container fluid className='container-fluid movie-app' style={{marginTop:"10vh"}}>
        <Row>
        <div className='row d-flex align-item-center mt-2 mb-2'>
          <MovieListHeading heading="Top Rated India"/>
        ...
        </Row>
        {
          !error&&Loading?(
            ...
            <div className='row'>
            <TopRatedIndiaComponent movies={movieList} AddToFavourite={AddToFavourite} removeFromFavourite={removeFromFavourite} favouriteChecker={favouriteChecker}/>
        </div>
          )
        }
        <hr />
        ...
        <div className='row'>
            <TopRatedIndiaComponent movies={favourite} AddToFavourite={AddToFavourite} removeFromFavourite={removeFromFavourite} favouriteChecker={favouriteChecker}/>
        </div>
      </Container>
  )
}
