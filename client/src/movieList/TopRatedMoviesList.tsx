...
import { TopRatedMoviesComponent } from '../movie-card/TopRatedMoviesComponent';
...
export const TopRatedMoviesList = () => {
  ...
  const fetchData = async(searchQuery:string)=>{
    try{
      const response = await axios.get(`http://localhost:3002/top-rated-movies?title_like=${searchQuery}`)
      ...
      <Container fluid className='container-fluid movie-app' style={{marginTop:"10vh"}}>
        ...
        </Row>
        {
          !error&&Loading?(
            ...
            <div className='row'>
            <TopRatedMoviesComponent movies={movieList} AddToFavourite={AddToFavourite} removeFromFavourite={removeFromFavourite} favouriteChecker={favouriteChecker}/>
        </div>
          )
        }
        <hr />
        ...
        <div className='row'>
            <TopRatedMoviesComponent movies={favourite} AddToFavourite={AddToFavourite} removeFromFavourite={removeFromFavourite} favouriteChecker={favouriteChecker}/>
        </div>
      </Container>
  )
}
