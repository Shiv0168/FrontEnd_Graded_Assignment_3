...
import { MovieInTheatersComponent } from '../movie-card/MovieInTheatersComponent';
...
export const MoviesInTheatersList = () => {
  ...
  const fetchData = async(searchQuery:string)=>{
    try{
      const response = await axios.get(`http://localhost:3002/movies-in-theaters?title_like=${searchQuery}`)
  ...
  return (
      <Container fluid className='movie-app' style={{marginTop:"10vh"}}>
        ...
        <div className='row d-flex align-item-center mt-2 mb-2'>
          <MovieListHeading heading="Movies In Theaters"/>
        ...
        </Row>
        {
          !error&&Loading?(
            ...
            <div className='row'>
            <MovieInTheatersComponent movies={movieList} AddToFavourite={AddToFavourite} removeFromFavourite={removeFromFavourite} favouriteChecker={favouriteChecker}/>
        </div>
          )
        }
        <hr />
        ...
        <div className='row'>
            <MovieInTheatersComponent movies={favourite} AddToFavourite={AddToFavourite} removeFromFavourite={removeFromFavourite} favouriteChecker={favouriteChecker}/>
        </div>
      </Container>
  )
}
